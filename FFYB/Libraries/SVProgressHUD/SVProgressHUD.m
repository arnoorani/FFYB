...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    UIAccessibilityPostNotification(UIAccessibilityScreenChangedNotification, nil);
    UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, string);
    //krishna duartion +0.10
    self.fadeOutTimer = [NSTimer timerWithTimeInterval:duration+0.10 target:self selector:@selector(dismiss) userInfo:nil repeats:NO];
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length + 0.1, 1.0);//*0.10
}
